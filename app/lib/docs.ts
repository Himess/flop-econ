...
export const CALCS: readonly CalcEntry[] = [
  ...
  },
  {
    ...
    body: "FLOP has no maximum supply — after five halvings the reward holds at 3 in perpetuity — so there is no fully-diluted point to anchor on, and the anchor year is explicit and adjustable. Genesis supply is the divisor, and it has forked twice in a month. D-0438 ratified 3,500,000,000 on 2026-09-10, superseding the D-0421/D-0435 pool sizes. Then a FLOP tokenomics graphic published the same day stated 4,400,000,000, with a validator airdrop of 1.20bn against Appendix A's 305,505,000 — roughly four times — and no ratifying decision. THE FIGURES ON THIS PAGE DIVIDE BY THE ANNOUNCED 4.4bn. That is an operator decision, not a reading of the specification: Appendix A remains the value of record, it is computed beside every figure, and no column derived from the announced number can ever read as ratified. Divide by Appendix A instead and the break-even valuation falls about 11% at the year-one anchor. Whichever you take, the whole difference is one line — the validator airdrop.",
